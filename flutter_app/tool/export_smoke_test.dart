...
void main(List<String> args) {
  if (args.isEmpty) {
    stderr.writeln('Usage: dart run tool/export_smoke_test.dart <raw-file> [out-dir]');
    exit(1);
  }
  final path = args[0];
  ...
  for (final format in ExportFormat.values) {
    ...
    final result = exportPhoto(
      ExportRequest(
        sourcePath: path,
        destPath: destPath,
        params: const RenderParams(temperature: 7000, exposure: 15, contrast: 10, vibrance: 25),
        format: format,
        quality: 92,
      ),
    );
    ...
    // ignore: avoid_print
    print('${format.label}: $size bytes in ${watch.elapsedMilliseconds}ms -> $destPath');
  }
}
