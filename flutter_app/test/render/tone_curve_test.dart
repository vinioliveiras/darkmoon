...
void main() {
  group('buildToneCurveLut', () {
    ...
    test('collinear points (not just the default curve) stay a straight line', () {
      const collinear = [
        CurvePoint(0, 0),
        CurvePoint(0.5, 0.5),
        CurvePoint(1, 1),
    ...
    test('an S-curve matches RapidRAW\'s monotone Hermite spline', () {
      ...
      expect(lut[64], closeTo(39, 1)); // x=0.251 -> y=0.151 (near control point)
      expect(lut[96], closeTo(80, 1)); // x=0.376 -> y=0.3147
      expect(lut[128], closeTo(128, 1)); // x=0.502 -> y=0.5028 (near control point)
